...
#### 相关视图
```sql
create or replace view public.auth_user_perm_view(id, saas_user_code, is_global, user_code, permission_code) as
...
```sql
create or replace view public.auth_user_router_view
            (id, name, menu_group, route_path, component, type, icon, parent_id, route_name, keep_alive,
             sort, visible, params, perm, has_perm, status, ctime, mtime, saas_user_code, role_code)
as
SELECT t5.id,
    t5.name,
    t5.menu_group,
